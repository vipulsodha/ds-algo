...
public class Solution {
    ...
    static int dfs(Node root, boolean[] processed) {

        int childCost  = 0;

        for (Node child : root.childs) {

            if(!processed[child.index]) {

                processed[child.index] = true;
                childCost = childCost + dfs(child, processed);


            }
        }

        int cuttingCost = Math.abs(totalCost - (childCost + root.data) - (childCost + root.data));


        if(cuttingCost < minimum) {
            minimum = cuttingCost;
        }

        return childCost + root.data;
    }
    ...
    static int cutTheTree(int[] data, int[][] edges) {
        ...
        boolean[] processed  = new boolean[data.length];
